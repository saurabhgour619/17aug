...
public class BookingController {
	...
	public ResponseEntity<Object> getResort(@PathVariable long rReservationNumber) {
		LOGGER.debug("BookingController:Debugging getResort method");
		LOGGER.info("BookingController:	Executing get Resort details");
		Resort getResort = bookingService.getResort(rReservationNumber);
		if (getResort != null) {
			LOGGER.info("Resort Booked for a Guest and bookedResort Entity returned.");
			return new ResponseEntity<Object>(getResort, HttpStatus.OK);
		} else {
			LOGGER.warn("Resort booking failed for a Guest.");
	...
	public ResponseEntity<Object> updateBookResort(@RequestBody Resort resort, @PathVariable long resortBookingId) {
		...
		if (updateResort != null) {
			...
			LOGGER.warn("Booked Resort for a Guest is not updated.Resort booking id not found");
	...
	public ResponseEntity<Object> getDining(@PathVariable long dReservationNumber) {
		LOGGER.debug("BookingController:Debugging getDining method");
		LOGGER.info("BookingController:	Executing get Dining details");
		Dining getDining = bookingService.getDining(dReservationNumber);
		if (getDining != null) {
			LOGGER.info("Resort Booked for a Guest and bookedResort Entity returned.");
			return new ResponseEntity<Object>(getDining, HttpStatus.OK);
		} else {
			LOGGER.warn("Resort booking failed for a Guest.");
